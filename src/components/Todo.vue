<template>
    <n-card class="shadow-md" :class="{ 'bg-[#f5f5f5]': store.theme === 'background' }">
        <n-collapse-item :title="todo.title" :name="todo.id">
            <template #header-extra>
                <i v-if="todo.pin" class="w-[10px] h-[10px] rounded-full bg-[#ff0000] mx-3"></i>
                <n-checkbox style="color: red" :checked="todo.done" size="large"
                    @click.stop="changeDone(todo.id, !todo.done)"></n-checkbox>
                ...
                <n-popover style="padding: 0" trigger="click" placement="left-start">
                    ...
                    <p class="text-center px-14 py-2 cursor-pointer" @click="changePin(todo.id, !todo.pin)">
                        {{ todo.pin ? "Unpin 📌" : "Pin 📌" }}
                    </p>
                    <hr>
                    <p class="text-center px-14 py-2 cursor-pointer" @click="updateTodo(todo.id)">Edit 🖍</p>
                    <hr>
                    <p class="text-center px-14 py-2 cursor-pointer" @click="store.deleteTodo({ id: todo.id })">Delete
                        🗑</p>
